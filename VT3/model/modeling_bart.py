# Code modified from vl-bart implementation (https://github.com/j-min/VL-T5/blob/main/VL-T5/src/modeling_bart.py)

import random
from transformers.models.bart.modeling_bart import (
    BartEncoder, BartDecoder,
    BartModel, BartForConditionalGeneration,
    BartConfig, shift_tokens_right, _expand_mask
)
import torch
...
class VisualEmbedding(nn.Module):
    def __init__(self, config, obj_order_embedding):
        ...
        if self.config.individual_vis_layer_norm:
            ...
            self.absolute_vis_pos_embedding = nn.Embedding(200, config.d_model) # TODO: hard coded max len of ViT visual tokens -> 197
    ...
    def forward(self, feats, img_order_ids=None, obj_order_ids=None):
        ...
        absolute_vis_pos_embedding = self.absolute_vis_pos_embedding(torch.arange(N, device=device)) # TODO: Check this if it is correctly used
...
class JointEncoder(BartEncoder):
    ...
    def forward(
        ...
        vis_embeds = self.visual_embedding(vis_feats, img_order_ids, obj_order_ids)
        V_L = vis_embeds.size(1) # len of visual tokens
...
class ModifiedVLBartModel(BartModel):
...
class ModifiedVLBart(BartForConditionalGeneration):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.model = ModifiedVLBartModel(config)
